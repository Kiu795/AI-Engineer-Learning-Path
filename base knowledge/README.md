...
## 第8周 — 大模型微调技术（LoRA、QLoRA、Tokenization + 中文分词 jieba）

### ※ 新增内容：中文分词（jieba）

- 了解中文分词的必要性（与英文空格分词区别）
- 掌握 jieba 的三种模式：精确模式、全模式、搜索模式
- 学习添加 **自定义词典** 与调节分词效果
- 实验：对两段中文文本进行分词 → 比较对 RAG chunking 的影响

| 天数 | 日预计时长 | 任务（输出/练习） |
| ---- | ---------- | ----------------- |
|      |            |                   |

| 周1  | 1–2h | **Tokenizer 深入讲解**：BPE、SentencePiece、WordPiece。理解文本切分的不同策略。输出：对比三者优劣的笔记。 |
| ---- | ---- | ------------------------------------------------------------ |
|      |      |                                                              |

| 周2  | 1–2h | **Tokenizer 实操**：用 HuggingFace Tokenizer 训练一个小 BPE tokenizer；测试对中文、英文混合文本的切分效果。 |
| ---- | ---- | ------------------------------------------------------------ |
|      |      |                                                              |

| 周3  | 1–2h | **LoRA 原理**：低秩分解、权重注入机制、训练参数减少的原因。输出：画 LoRA 结构示意图。 |
| ---- | ---- | ------------------------------------------------------------ |
|      |      |                                                              |

| 周4  | 1–2h | **QLoRA 原理**：4-bit quantization、NF4、double quant。理解为何能节省显存。 |
| ---- | ---- | ------------------------------------------------------------ |
|      |      |                                                              |

| 周5  | 1–2h | **LoRA/QLoRA 实战**：对一个小型模型（如 GPT2-small）做简单指令微调。输出：loss 曲线 + 推理示例。 |
| ---- | ---- | ------------------------------------------------------------ |
|      |      |                                                              |

| 周6  | 1–2h | **中文分词（jieba）基础**：精确模式、全模式、搜索模式；如何选择适合 RAG 的分词模式。练：对一段新闻文本分词并对比模式差异。 |
| ---- | ---- | ------------------------------------------------------------ |
|      |      |                                                              |

| 周7  | 1–2h | **高级 jieba 技巧 + RAG 预处理准备**：添加自定义词典、调整词频；对两篇文档做分词 → chunk → embedding，并分析 chunk 粒度影响。 |
| ---- | ---- | ------------------------------------------------------------ |
|      |      |                                                              |
